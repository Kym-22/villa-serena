...
export default function ModuloSwitcher({
  ...
  // Con 6 módulos la lista de "otros" llega a 5 botones: se limita la altura y
  // se hace desplazable para que no aplaste la navegación en pantallas bajas.
  return (
    <div className="px-3 pb-3 shrink-0 space-y-2 max-h-[42vh] overflow-y-auto">
      {otros.map(m => (
        <button
          key={m.id}
          onClick={() => onCambiar(m.id)}
